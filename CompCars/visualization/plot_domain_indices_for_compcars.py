...
def draw_2_div(data, num_domain):
    ...
    VIEW = ['Front (F)', 'Front-side (FS)','Side (S)', 'Rear-side (RS)', 'Rear (R)']
    ...
    ax.set_title("{} (Colors Indicate Viewpoints)".format(r"$\beta$"), y=1.0, pad = 11)
    ax.title.set_size(title_size)
    ax.set_aspect(aspect_ratio)
    plt.savefig("{}/{}_colors_indicate_viewpoints.pdf".format(save_folder, "Beta"),bbox_inches='tight',dpi=300,format='pdf')
    # plt.show()
    plt.clf()
    ...
    ax.title.set_size(title_size)
    ax.set_aspect(aspect_ratio)
    plt.savefig("{}/{}_colors_indicate_YOMs.pdf".format(save_folder, "Beta"),bbox_inches='tight',dpi=300,format='pdf')
    # plt.show()
    plt.clf()
